...
from ndn_hydra.repo.modules.favor_calculator import favor_cal, entropy_weight
...
class MainLoop:
    ...
    def run_calculations(self):
        config_bw = [100]  # bandwidth -> Config file
        config_c1 = np.array([1500])  # storage cost -> Config file
        config_rtt = [
            [0, 38.27, 113.00, 19.36, 7.87, 25.25, 20.39, 156.20, 36.09, 5.42],
            [38.27, 0, 141.10, 47.16, 35.50, 28.90, 48.14, 184.00, 0.18, 33.10],
            [113.00, 141.1, 0, 118.00, 111.00, 88.35, 119.00, 254.00, 117.00, 108.00],
            [19.36, 47.16, 118, 0, 16.67, 29.51, 1.04, 137.00, 44.81, 14.21],
            [7.87, 35.5, 111.00, 16.67, 0, 22.35, 17.51, 153.00, 33.18, 2.51],
            [25.25, 28.9, 88.35, 29.51, 22.35, 0, 30.32, 166.00, 28.81, 19.91],
            [20.39, 48.14, 119, 1.04, 17.51, 30.32, 0, 138.00, 59.21, 15.14],
            [156.2, 184, 254, 137, 153, 166, 138, 0, 188, 151],
            [36.09, 0.18, 117, 44.81, 33.18, 28.81, 59.21, 188, 0, 30.74],
            [5.42, 33.1, 108, 14.21, 2.51, 19.91, 15.14, 151, 30.74, 0],
        ]  # rtt -> Config file

        config_v = [6.25]  # r/w speed -> Not in config
        config_c2 = np.array([530.80])  # hard drive cost -> No in config
        config_C_N = 5.0  # filesize -> Not in config
        config_popularity = [0.2, 0.1, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]  # popularity -> Not in config

        # Step 1: Run the optimization without any policies
        X, F = favor_cal(rtt=config_rtt,
                         bandwidth=config_bw,
                         v=config_v,
                         c1=config_c1,
                         c2=config_c2,
                         C_N=config_C_N,
                         popularity=config_popularity,
                         user_node="TACC",
                         weights=np.ones(4) / 4)  # Equal weights for initial run

        # Step 2: Calculate the entropy weights

        weights = entropy_weight(F)
        print("Entropy Weights:", weights)

        # Step 3: Check if the user has specified any policies
        user_policies = True

        if user_policies:
            # Step 4: Allow the user to customize the weights
            user_customization = {0: 10, 1: -5, 2: 0, 3: -5}  # obj1: +10%

            # Step 5: Adjust the weights based on user customization
            for objective, percentage in user_customization.items():
                weights[objective] *= (1 + percentage / 100)

            # Step 6: Normalize the adjusted weights
            weights /= np.sum(weights)

        print("Adjusted Weights:", weights)

        # Step 7: Run the optimization with the adjusted weights
        best_solution, F = favor_cal(rtt=config_rtt,
                                     bandwidth=config_bw,
                                     v=config_v,
                                     c1=config_c1,
                                     c2=config_c2,
        ...
        # TODO: What if I want to extract the current node favor value?
        # Extract the favor value for current node (TACC)
        nodes = ["MICH", "UTAH", "TACC", "WASH", "NCSA", "DALL", "MAX", "MASS", "SALT", "STAR"]
        favor = best_solution[nodes.index("TACC")]
        print("Favor for TACC:", favor)
    ...
    def send_heartbeat(self):
        ...
        favor = self.global_view.get_node(heartbeat_message.node_name)['favor']
        heartbeat_message.favor_weights = entropy_weight(favor)
    ...
    def claim_heartbeat_message(self):
        ...
        for backupable_file in backupable_files:
            ...
            claim_message.favor = self.global_view.get_node(claim_message.node_name)['favor'].enconde()
